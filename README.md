...
>
> **Do not use BORN-256 to protect production systems or sensitive real-world data.**
...
The experimental key schedule generates one 256-bit round key for each of the 16 rounds.
...
|---|---:|
| Samples | 1000 |
| Minimum output difference | 105 bits |
| Maximum output difference | 149 bits |
| Average output difference | 128.59 / 256 |
| Average percentage | 50.23% |

This experiment measures diffusion of arbitrary input differences.
...
The same plaintext and key consistently produced the same ciphertext.
...
> These results are experimental measurements only. They do not constitute a cryptographic security proof.
...
Changing a single key bit produced substantial ciphertext differences in the tested experiments.
...
Repeated encryption and decryption operations behave consistently across the tested samples.
...
The results should be interpreted as experimental measurements of the current implementation and its observed behavior.
...
Each major component is tested before being integrated into the complete construction.
...
9. Cryptographic research and learning
